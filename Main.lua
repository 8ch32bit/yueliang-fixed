-- 8ch_32bit was here to do some bug fixes :)

local function Ldexp(mantissa, exponent) -- for Lua 5.3+ support
	return mantissa * (2 ^ exponent);
end;

local function Frexp(x) -- for Lua 5.3+ support/optimization
	if x == 0 then
		return 0, 0;
	end;

	local IsNegative = x < 0;
	local New = math.abs(x);

	local Mantissa, Exponent = 0, 0;

	Exponent = math.floor(math.log(New, 2)) + 1;
	Mantissa = New / 2 ^ Exponent;

	if math.abs(Mantissa) >= 1 then
		Mantissa = Mantissa / 2;
	end;
...
do
	...
	function luaZ:init(reader, data)
		local z = { reader = reader, data = data or "", n = 0, p = 0 };
	...
	luaP.MAXARG_Bx  = Ldexp(1, luaP.SIZE_Bx) - 1
	luaP.MAXARG_sBx = math.floor(luaP.MAXARG_Bx / 2)  -- 'sBx' is signed

	luaP.MAXARG_A = Ldexp(1, luaP.SIZE_A) - 1
	luaP.MAXARG_B = Ldexp(1, luaP.SIZE_B) - 1
	luaP.MAXARG_C = Ldexp(1, luaP.SIZE_C) - 1
	...
	luaP.BITRK = Ldexp(1, luaP.SIZE_B - 1)
	...
	function luaU:make_setS()
		...
		local function writer(s, _buff)  -- chunk writer
			if not s then return 0; end;

			_buff.data = _buff.data .. s;
	...
	function luaU:make_setF(filename)
		local buff = { h = io.open(filename, "wb") };

		if not buff.h then return nil end;

		local function writer(s, _buff)  -- chunk writer
			if not _buff.h then return 0; end;

			if not s then
				if _buff.h:close()  then return 0; end;
			else
				if _buff.h:write(s) then return 0; end;
			end;
